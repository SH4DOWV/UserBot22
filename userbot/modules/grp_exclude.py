...
async def exclude_grp(excl):
    if not is_mongo_alive():
        await excl.edit("`Connessione al Database fallita!`")
        return
    ...
    await excl.edit(
        f"`Questa chat (ID: {chat_id}, tipo di esclusione: {exclude_type}) è stata aggiunta alla lista delle Chat Escluse!`"
    )
...
async def unexclude_grp(excl):
    ...
    await excl.edit(
        f"`Questa chat (ID: {chat_id}) è stata rimossa dalle Chat Escluse!`"
    )
...
async def listexclude_grp(excl):
    ...
    if resp != "":
        resp = f"`Lista delle Chat escluse:`\n\n{resp}"
    else:
        resp = f"`Non ci sono chat escluse.`"
